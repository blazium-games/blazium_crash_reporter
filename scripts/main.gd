...
@onready var api_key_edit: LineEdit = $Margin/VBox/Settings/ApiKey
...
func _on_selected(index: int) -> void:
	...
	detail.text = JSON.stringify(row.get("metadata", {}), "\t")
...
func _on_send() -> void:
	...
	var err := await Uploader.upload(url, api_key_edit.text, crash_dir, id, include_logs.button_pressed)
	if err == OK:
		status.text = "Uploaded %s. Fetching stackwalk..." % id
		var server_detail := await Uploader.fetch_server_detail(url, api_key_edit.text, id)
